...
local void
local SSPVPLoaded = IsAddOnLoaded("SSPVP3") or IsAddOnLoaded("SSPVP")
...
function LeaPlusLC:Isolated()
    ...
    if LeaPlusLC["ShowPlayerChain"] == "On" and not LeaLockList["ShowPlayerChain"] then
        ...
        local function SetChainStyle()
            ...
            if not IsAddOnLoaded("SSPVP3") and not IsAddOnLoaded("SSPVP") then
                if chain == 1 then
                    -- Rare
...
function LeaPlusLC:Player()
    ...
    if LeaPlusLC["ShowReadyTimer"] == "On" then
        ...
        do
            ...
            local BGqueue_OnShow
            if StaticPopupDialogs["CONFIRM_BATTLEFIELD_ENTRY"] then
                BGqueue_OnShow = StaticPopupDialogs["CONFIRM_BATTLEFIELD_ENTRY"].OnShow
            ...
            local BGQueue_OnAccept, BGQueue_OnCancel

            if StaticPopupDialogs["CONFIRM_BATTLEFIELD_ENTRY"] then
                BGQueue_OnAccept = StaticPopupDialogs["CONFIRM_BATTLEFIELD_ENTRY"].OnAccept
                BGQueue_OnCancel = StaticPopupDialogs["CONFIRM_BATTLEFIELD_ENTRY"].OnCancel
